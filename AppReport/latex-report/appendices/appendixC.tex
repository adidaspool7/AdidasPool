...
(\chapref{cap:analysis}) and the validation (\chapref{cap:validation}).
\secref{app:survey-meeting} summarises the client engagement that
produced the pain points; \secref{app:survey-instrument} reproduces the
...
2026) with adidas Business Services in Porto, the shared-services centre
profiled in \secref{sec:context}. The original client brief
...
\begin{table}[ht]
	...
	\begin{tabular}{@{}l p{8.5cm}@{}}
		...
		May--Jul 2026 & Final report and presentation. \\
		\bottomrule
	\end{tabular}
\end{table}

These meetings established the Scrum-with-the-client cadence described in
\secref{sec:method-scrum}: the author acted as Product Owner, translating
client conversations into a prioritised backlog and demonstrating increments
on the deployed environment. Nine pain points emerged from the engagement.
They are reproduced below in the client's own framing --- the raw source
later consolidated into the overview of \tabref{tab:pain-points-overview} ---
with each mapping to one or more platform modules in
\tabref{tab:survey-mapping}.
...
evidence for the corresponding module. The nine pain points were expanded
into a fourteen-item instrument, several pain points contributing more than
one question so that a single friction could be probed from complementary
angles. The items are listed below exactly as administered and numbered to
match the response rows of \tabref{tab:survey-responses}.

\subsection*{Pain-point questionnaire}

\begin{enumerate}
	\item \textit{Organising candidate information.}\\
	``What tool does your HR team primarily use to store and manage candidate
	information during a recruitment process?''
	(Excel / Google Sheets / An ATS, e.g.\ Workday or Greenhouse / Email
	folders / Shared documents / No structured system / Other)
	...
	(Check manually in our records / Rely on the candidate declaring it / No
	way to verify / Automated system / Other)

	\item \textit{Data-retention loss.}\\
	``Has your team ever lost useful information about a past candidate because
	data-retention policies required it to be deleted?''
	(Yes, frequently / Yes, occasionally / Rarely / Never / Not sure)
	...
	dedicated assessment round / No formal check --- we rely on the CV / Other)
	...
	(Individual emails written manually / Automated emails via an ATS / Phone
	calls / No structured process / Other)

	\item \textit{Re-engaging past candidates.}\\
	``Has your team ever had to re-engage a candidate for a new position after
	they were not selected for a previous one? If yes, how did you manage that
	outreach?''
	(Yes, and it was easy / Yes, but it was difficult to find their
	information / No, we do not reuse past candidates / Not applicable / Other)

	\item \textit{Saving time on CV analysis.}\\
	``When screening a batch of different CVs, how long does it typically take
	your team to extract key information (e.g.\ location, years of experience,
	languages, contacts) from each individual CV?''
	(Under 1~min / 1--3~min / 3--5~min / More than 5~min)

	\item \textit{Unsuitable-CV frequency.}\\
	``How often do you open a CV only to find immediately that the candidate
	does not meet a basic requirement (e.g.\ wrong location, missing
	language)?''
	...
	recruitment pipeline --- for example, how many candidates are at each
	stage, or how many applications were received per job opening?''
	(Very easy --- I have a dashboard / Possible but takes time to compile /
	Very difficult / No visibility / Other)

	\item \textit{Where to focus recruitment efforts.}\\
	``When planning a new recruitment campaign, how does your team identify
	which skills, languages or profiles are most in demand across the
	business?''
	(Analyse data from past recruitments / Rely on input from managers / Done
	intuitively / No structured process / Other)

	\item \textit{Share of unqualified CVs.}\\
	``Roughly what percentage of CVs you receive for a typical open position
	would you say clearly do not meet the minimum requirements (wrong
	location, missing language, insufficient experience)?''
	(Less than 10\% / 10--30\% / 30--60\% / More than 60\%)
	...
	manual tasks?''
	(1 = they add very little value $\to$ 5 = they significantly boost my
	productivity)
	...
	experience?''
	(Keep their profile for future openings / Inform them of other
	opportunities proactively / Rejected and removed / No structured process /
	Other)

	\item \textit{Cross-team coordination.}\\
	``When multiple HR team members are involved in the same candidate's
	process, how easy is it to know what actions have already been taken ---
	for example, who contacted them, when, and what was discussed?''
	(Very easy / Possible but requires effort / Difficult --- we rely on memory
	or informal communication / No way to track this / Other)
\end{enumerate}

\subsection*{Original four-question bank (February draft)}
...
\secref{sec:val-survey}. \tabref{tab:survey-responses} reproduces the
...
\secref{sec:val-survey}: the floor productivity rating (Q12) from a
