...
| # | Task | Status | Notes |
|---|---|---|---|
| 4.1 | Pre-interview dropdown: select "Language Assessment" or "Technical Assessment" | ⬜ | Added before the interview popup opens |
| 4.2 | Language Assessment mode — CEFR rubric scoring | ⬜ | Modify FastAPI system prompt; evaluate grammar, vocabulary, fluency |
| 4.3 | Technical Assessment mode — AI Skill Validation | ⬜ | Current behavior, no changes needed |
| 4.4 | Fix topic mixing — questions should focus on one topic at a time | ⬜ | Update FastAPI system prompt; enforce single-topic constraint |
| 4.5 | Pass selected mode to FastAPI `/interview/start` | ⬜ | Add `mode` field to start request body |
| 4.6 | Display mode badge in interview UI | ⬜ | Language or Technical label in popup header |
